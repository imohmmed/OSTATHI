/**
 * شاشة محادثة الأستاذ / المساعد مع طالب معين
...
interface PendingAttachment {
  uri: string; name: string; type: string; kind: 'image' | 'file';
}
...
export default function InboxStudentPage() {
  ...
  const teacherName = decodeURIComponent(params.teacherName ?? (user?.role === 'assistant' ? (user as any).teacherName ?? '' : user?.fullName ?? ''));

  const [draft, setDraft] = useState('');
  const [pending, setPending] = useState<PendingAttachment | null>(null);
  const [uploading, setUploading] = useState(false);

  const senderType: 'teacher' | 'assistant' = user?.role === 'assistant' ? 'assistant' : 'teacher';
  const senderName = teacherName;
  ...
  }, [draft, pending, user, sendMsg, senderType, senderName]);

  const isBusy = uploading || sendMsg.isPending;
  const canSend = (draft.trim().length > 0 || !!pending) && !isBusy;
  const formatTime = (iso: string) => new Date(iso).toLocaleTimeString('ar', { hour: '2-digit', minute: '2-digit' });
    ...
    <>
      ...
      >
        ...
        <View style={[styles.replierBar, { backgroundColor: `${colors.gold}15`, borderBottomColor: c.border }]}>
          <Ionicons
            name={senderType === 'assistant' ? 'person-circle-outline' : 'person-circle'}
            size={15}
            color={colors.gold}
          />
        ...
        {isLoading ? (
          ...
          <FlatList
            ...
            renderItem={({ item }) => {
              const isTeacher = item.senderType === 'teacher' || item.senderType === 'assistant';
              const opacity = isOptimistic(item) ? 0.6 : 1;
              return (
                <View style={[styles.row, isTeacher ? styles.rowMe : styles.rowThem]}>
                  {isTeacher ? (
                    <View style={[styles.bubble, styles.bubbleMe, { backgroundColor: colors.navy, opacity }]}>
                      ...
                      {item.attachmentUrl && <AttachmentBubble url={item.attachmentUrl} type={item.attachmentType} name={item.attachmentName} textColor="#fff" />}
                      {item.text ? <Text style={[styles.msgText, { color: '#fff' }]}>{item.text}</Text> : null}
                      <Text style={[styles.timeText, { color: 'rgba(255,255,255,0.45)' }]}>
                        {isOptimistic(item) ? '...' : formatTime(item.createdAt)}
                      </Text>
                    </View>
                  ) : (
                    <View style={[styles.bubble, styles.bubbleThem, { backgroundColor: c.muted, borderColor: c.border }]}>
                      <View style={styles.senderRow}>
                        <Ionicons name="person-outline" size={13} color={c.mutedForeground} />
                        <Text style={[styles.senderName, { color: c.mutedForeground }]}>{studentName}</Text>
                      </View>
                      {item.attachmentUrl && <AttachmentBubble url={item.attachmentUrl} type={item.attachmentType} name={item.attachmentName} textColor={c.foreground} />}
                      {item.text ? <Text style={[styles.msgText, { color: c.foreground }]}>{item.text}</Text> : null}
            ...
            ListEmptyComponent={
              <View style={styles.center}>
                ...
                <Text style={[{ color: c.mutedForeground, fontFamily: 'Tajawal_400Regular', fontSize: 14 * fs, textAlign: 'center' }]}>
                  لا توجد رسائل من {studentName} بعد
                </Text>
              </View>
            }
          />
